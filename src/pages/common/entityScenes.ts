...
import { kubernetesEventsQuery, kubernetesLogsQuery, kubernetesWarningEventsQuery, kubernetesWarningsAndErrorsQuery } from '../../queries/logs';
...
import { networkDrops, networkErrors, networkReceive, networkReceivePackets, networkTransmit, networkTransmitPackets } from '../../queries/network';
...
import { daemonSetReadiness, deploymentReadiness, pendingPods, podRestartCount, podsNotReady, podWaitingReasons, statefulSetReadiness } from '../../queries/scheduling';
import {
  ...
  return pageScene([
    ...
    row(
      [
        ...
        item(timeseriesPanel('Memory requests / capacity', memoryRequestsToCapacity(scope), 'percentunit'), '50%', 300),
      ],
      320
    ),
    row(
  ...
  return pageScene([
    ...
    row(
      [
        ...
        item(timeseriesPanel('Memory p95 / requests', memoryWorkingSetP95ToRequests(scope), 'percentunit'), '25%', 280),
      ],
      300
    ),
    row(
  ...
  return pageScene([
    row(
      [
        ...
        item(timeseriesPanel('Node filesystem utilization', nodeFilesystemUtilization(scope), 'percentunit'), '34%', 300),
      ],
      320
    ),
    row(
